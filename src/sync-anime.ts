...
async function syncAll() {
    try {
        ...
        while (currentPage < totalPages) {
            if (currentPage != 0) {
                let nextPageData = await downloadAnimeListHTML(root, currentPage);
...
async function downloadAnimeListHTML(root: string, page: number) {
    let url = root + `/list/?31${page ? `-${page + 1}` : ''}.html`;
