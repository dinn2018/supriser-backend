...
retry(syncLatestCartoon);
later.setInterval(syncLatestCartoon, sched);
async function syncLatestCartoon() {
    ...
    for (let cartoon of cartoons) {
        ...
        let cartoonNum: any = {};
        $('div.pure-g div#content li a').each((index, elem) => {
            if (index = 0) {
                let href = `${cartoonHref}${elem.attribs['href']}`;
                let name = elem.attribs['title'];
                cartoonNum = { href, name };
            }
        })
...
async function loadHTML(href: string): Promise<string> {
    let jsPath = path.join(__dirname, 'cronPhantom.js');
